...
roc_range = np.arange(-4000, 4000, 0.1) # ft/min
...
# states = np.arange(0, 6, 0.01)
#
# alt_gnd = fuzz.zmf(alt_range, 0, 100) # Changed from 0 to 100
# alt_lo = fuzz.gaussmf(alt_range, 3000, 1500)
# alt_hi = fuzz.gaussmf(alt_range, 12000, 6000)
#
# roc_zero = fuzz.gaussmf(roc_range, 0, 0.05)
# roc_plus = fuzz.smf(roc_range, 0.5, 5)
# roc_minus = fuzz.zmf(roc_range, -5, -0.05)
#
# spd_hi = fuzz.gaussmf(spd_range, 300, 75)
# spd_md = fuzz.gaussmf(spd_range, 200, 50)
# spd_lo = fuzz.gaussmf(spd_range, 0, 25)
...
def fuzzylabels(ts, alts, spds, rocs, twindow=30, cluster=False):
    """
    ...
    if cluster:
        X = np.array([ts, alts, spds, rocs]).transpose()
        # X = np.array([ts, alts, spds]).transpose()
        MinMaxScaler(feature_range=(0, 10), copy=False).fit_transform(X)
        # print(X[:2])
        cluster = DBSCAN(eps=0.2, min_samples=10).fit(X)
        labs = cluster.labels_
        max_lab = max(labs)
        # print("length: " + str(len(twindows)))
        twindows = np.where(labs == -1, ts//twindow +max_lab, labs)
        l, c = np.unique(twindows, return_counts=True)
        print(f"labels: {l}")
        print(f"label counts: {c}")
        fig, ax = plt.subplots(3)
        ax[0].scatter(ts, alts, s=1, c=twindows, cmap='viridis')
        ax[1].scatter(ts, spds, s=1, c=twindows, cmap='viridis')
        ax[2].scatter(ts, rocs, s=1, c=twindows, cmap='viridis')
        plt.show()
        print(f"after clustering: {len(twindows)}")
    ...
    for tw in range(0, int(max(twindows))):
        ...
        if len(idxchk) > 0:
            ...
            # print(f"End of labeling {len(labels)}")
            # print(f"idxchk : {idxchk[0]}, {idxchk[-1]}")
            # print(label)
            labels[idxchk[0]: (idxchk[-1] + 1)] = label
            # print(f"Before return {len(labels)}")
    # labels['LV'] = 'LVL'
    # labels = l(labels)
    return labels
